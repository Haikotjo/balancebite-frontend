...
const Camera = ({ onCapture, disabled }) => {
    ...
    const handleCapture = () => {
        ...
        setCapturedImage(imageSrc); // Sla foto lokaal op
        if (onCapture) {
            onCapture(imageSrc); // Stuur foto naar parent
        }
        setShowCamera(false);
    };

    const handleReset = () => {
        setCapturedImage(null); // Wis lokaal opgeslagen foto
        if (onCapture) {
            onCapture(null); // Stuur null terug naar parent
        }
    };
