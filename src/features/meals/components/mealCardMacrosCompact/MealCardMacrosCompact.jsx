...
const MealCardMacrosCompact = ({
                                   macros,
                                   vertical = false,
                                   keys = ["Calories", "Protein", "Carbs", "Fats"],
                                   iconSize = 18,
                                   textClassName = "",
                                   className = "",
                                   rowClassName,
                                   itemClassName = "bg-black/55 backdrop-blur-sm rounded-md ",
                               }) => {
    const rowBase = vertical
        ? "inline-flex flex-col items-stretch gap-2"
        : "grid grid-cols-2 gap-2 w-max";
            ...
            <CustomBox className={rowClassName || rowBase}>
                {keys.map((key) => {
                    ...
                    return (
                        <CustomBox
                            key={key}
                            className={`flex items-center justify-center gap-2 px-2 py-1 ${itemClassName}`}
                        >
